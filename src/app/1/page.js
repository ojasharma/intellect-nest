...
const getCurrentPhase = (scrollPercentage) => {
  const ranges = [
    [0, 30],
    [30, 37],
    [37, 44],
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    if (scrollPercentage >= start && scrollPercentage < end) return i;
  }
...
export default function HomePage() {
  ...
  useEffect(() => {
    const unsubscribe = useScrollStore.subscribe((state) => {
      ...
      const currentPhase = getCurrentPhase(scrollPercentage);

      setUiState((prev) => ({
        ...prev,
        instructionalFade:
          scrollPercentage >= 10 && scrollPercentage <= 32
            ? "fade-in"
            : "fade-out",
        scrollIndicatorFade: scrollPercentage > 5 ? "fade-out" : "fade-in",
        greatMoveFade: currentPhase === 3 ? "fade-in" : "fade-out",
        rookMoveFade: currentPhase === 5 ? "fade-in" : "fade-out",
        pawnMoveFade: currentPhase === 7 ? "fade-in" : "fade-out",
        finalMoveFade: currentPhase === 8 ? "fade-in" : "fade-out",
        isStatsVisible: currentPhase >= 4,
  ...
  return (
    <>
      <main className="relative h-screen flex flex-col items-center overflow-hidden">
        ...
        <Scene scrollToPercent={scrollToPercent} totalPhases={11} />
        <UIOverlay />
